...
{
    public partial class LoginMil : Form
    {
        ...
        private void Login_Load(object sender, EventArgs e)
        {
            ...
            RecordIt("starting from Login_Load");
        }
        ...
        void wb_DocumentCompleted(object sender, EventArgs e)
        {
            ...
            RecordIt("Doc comp " + wb.Url.AbsoluteUri + " and count = "+count.ToString());
            if (wb.Url.AbsoluteUri.Contains("logout"))
            {
                RecordIt("Doc comp redirected to " + authUrl);
                wb.Navigate(authUrl);
                return;
            }
            if (count > 0)
            {
                ...
                if ( loginCookie != null && loginCookie.ToUpper().Contains("SESSION_ID") /*  && !wb.IsBusy */)
                {
                    ...
                    //bool doIt = (fpos > 0 && (qpos < 0 || fpos < qpos)) || (loginAttempts > 0);
