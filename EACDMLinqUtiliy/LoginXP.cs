...
namespace EACDMLinqUtiliy
{
    public partial class LoginXP : Form
    {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            if (passwd.Equals("4BF55D82E331070C2B48EEAF7299CF15"))
            {
        ...
        void wb_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            ...
            int maxclicked = (BbQuery.BbUrl.Contains("maryland") /*|| BbQuery.BbUrl.Contains("rccc") */) ? 3 : 2;
            if (clicked < maxclicked || tryingToClick < 3)
            {
                if (ClickIt())
                {
                    ...
                    {
                        Uri d = new Uri(BbQuery.BbUrl);
                        Uri domain = new Uri(d.Scheme + "://" + d.Host);
                        UpdateCookies(QTIUtility.wbHelper.GetUriCookieContainer(domain), domain);
                        Debug.WriteLine("cookies: " + getAllCookies(domain));
                        if (Check(username, getCookie("session_id", domain), false))
                        {
        ...
        private bool ClickIt()
        {
            ...
            if (wb.Document.Window.Frames.Count > 0 && !(BbQuery.BbUrl.ToLower().Contains("cuny") || BbQuery.BbUrl.ToLower().Contains("gtc")))
            {
                for (int i = 0; i < wb.Document.Window.Frames.Count; i++)
                {
                    ...
                    button = null;
                    HtmlElementCollection c = doc.GetElementsByTagName("INPUT");
                    if (c.Count > 2)
                    {
                        ok = traverseInputs(doc.GetElementsByTagName("INPUT"), username, tbPassword.Text.Trim(), 0);
                    }
                    if (ok)
                    {
                        break;
            ...
            else
            {
                ...
                button = null;
                HtmlElementCollection c = doc.GetElementsByTagName("INPUT");
                Debug.WriteLine("inputs " + c.Count.ToString());
                if (c.Count > 2) { 
                ok = traverseInputs(doc.GetElementsByTagName("INPUT"), username, tbPassword.Text.Trim(), 0);
                }
            }
        ...
        private bool traverseInputs(HtmlElementCollection c, string username, string password, int retValue)
        {
            foreach (HtmlElement n in c)
            {
        ...
        private void Login_Load(object sender, EventArgs e)
        {
            ...
            //if (!(BbQuery.BbUrl.Contains("owens") || BbQuery.BbUrl.ToLower().Contains("genesee") || BbQuery.BbUrl.ToLower().Contains("umaryland") || BbQuery.BbUrl.ToLower().Contains("rccc")))
            //{
            //    BbLogout();
            //}
        }
            ...
            if (!special)
            {
                ...
                if (user != null && user.Rows.Count > 0 && !user.TableName.Equals("error"))
                {
                    ...
                    foreach (DataRow dr in user.Rows)
                    {
                        ...
                        //{
                        //    // Debug.WriteLine("Url " + dr["url"].ToString() + " " + dr["md5"].ToString());
                        //    if (ExistingMD5s != null && ExistingMD5s.Contains(dr["md5"].ToString()))
                        //    {
                        //        continue;
                        //    }
                        //}
                        ...
                        else
                        {
                            ...
                            BbQuery.cc.Add(new Uri(authUrl), c);
                            //if (!(BbQuery.BbUrl.Contains("maryland") || BbQuery.BbUrl.ToLower().Contains("genesee") || BbQuery.BbUrl.Contains("rccc")))
                            //{
                            //    BbLogout();
                            //}
